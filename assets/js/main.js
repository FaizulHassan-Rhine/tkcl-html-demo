...
(function ($) {
  ...
  // Our Services At A Glance Section - Swiper, Before/After, Tabs
  $(document).ready(function () {
    // Swiper for services glance
    if ($('.services-glance-swiper').length) {
      var glanceSwiper = new Swiper('.services-glance-swiper', {
        slidesPerView: 1,
        loop: true,
        navigation: {
          nextEl: '.glance-arrow-next',
          prevEl: '.glance-arrow-prev',
        },
        pagination: {
          el: '.services-glance-pagination',
          clickable: true,
        },
      });
    }

    // Before/After Slider Logic (lightweight, no external lib)
    function initBeforeAfterSlider($slider) {
      var $before = $slider.find('.before-img');
      var $after = $slider.find('.after-img');
      var $handle = $slider.find('.before-after-handle');
      var $circle = $slider.find('.before-after-circle');
      var dragging = false;
      var sliderOffset = 0;
      var sliderWidth = 0;
      ...
      function setPosition(percent) {
        ...
        var left = percent * 100;
        $after.css('clip-path', 'inset(0 0 0 ' + left + '%)');
        $handle.css('left', left + '%');
        $circle.css('left', 'calc(' + left + '% - 20px)');
      }

      function onMove(e) {
        if (!dragging) return;
        var pageX = e.type.startsWith('touch') ? e.originalEvent.touches[0].pageX : e.pageX;
        var percent = (pageX - sliderOffset) / sliderWidth;
        setPosition(percent);
      }
      ...
      $slider.on('click', function (e) {
        var pageX = e.type.startsWith('touch') ? e.originalEvent.touches[0].pageX : e.pageX;
        var offset = $slider.offset();
        var percent = (pageX - offset.left) / $slider.width();
        setPosition(percent);
      });
    ...
    // Init before/after for each slide
    $('.services-glance-swiper .swiper-slide').each(function () {
      var $slider = $(this).find('.before-after-slider');
      if ($slider.length) {
        initBeforeAfterSlider($slider);
      }
    });
    // Also re-init on slide change (for Swiper's looped slides)
    if (typeof glanceSwiper !== 'undefined') {
      glanceSwiper.on('slideChangeTransitionEnd', function () {
        $('.services-glance-swiper .swiper-slide').each(function () {
          var $slider = $(this).find('.before-after-slider');
          if ($slider.length) {
            initBeforeAfterSlider($slider);
    ...
    // Tab switching logic
    $('.glance-main-tab').on('click', function () {
      var tab = $(this).data('tab');
      $('.glance-main-tab').removeClass('active');
      $(this).addClass('active');
      $('.glance-sub-tab').hide().removeClass('active');
      $('.glance-sub-tab[data-main="' + tab + '"]').show();
      // Activate first sub-tab for this main tab
      $('.glance-sub-tab[data-main="' + tab + '"]').first().addClass('active');
    });
    $('.glance-sub-tab').on('click', function () {
      var main = $(this).data('main');
      $('.glance-sub-tab[data-main="' + main + '"]').removeClass('active');
      $(this).addClass('active');
      // Optionally, you can sync the carousel to the sub-service here
    });
  });
